...
p = uipanel(gcf, 'Units', 'pixels', 'FontWeight', 'bold', ...
    ...
    'Visible', 'off', ...
    'Title', 'MS Midpoint Displacement', ...
...
uicontrol(p, 'Style', 'pushbutton', ...
    'String', 'Calculate', ...
    'Callback', @MSMidpointDisplacement_Calculate, ...
    'Position', [135 ph-112 70 22]);
% Save ui elements as 'UserData' of the panel
set(p, 'UserData', ui);

function MSMidpointDisplacement_Calculate(hObject, eventdata)
...
% maxStepNum
maxStepNum = floor(max(cellfun(@length, data)-1)/2);
step = 2*step;

% Fitting equation and options for the current processing
ft = fittype('x^3/(48*lp)');
fo = fitoptions('Method', 'NonlinearLeastSquares', 'Startpoint', 10, ...
    'Lower', 0, 'Upper', Inf);
...
if strcmpi(procLength_str, 'overview')
    ...
    % Calculate MS midpoint displacement vs Internal contour length
    [xdata, ydata, count] = algorithm.MSMD(data, step, procStepNum);
    ...
    if isempty(procLength) || isnan(procLength) || procLength < 3*step
        ...
        % round procLength to an integer number of 2*step, but < than maxStepNum
    ...
    if toSave
        [fileName, filePath] = uiputfile('*.txt', ...
            'Save As', ['MSMD_' dataName_str(1:end-4) '.txt']);
        if fileName ~= 0
            % Save data
...
ylabel('MS midpoint displacement, \langleu^2\rangle (nm^2)');
title(dataName_str, 'Interpreter', 'none');
lp = sprintf('%.2f', cv);

function gaborArray = gaborFilterBankMod(u,v,m,n)
...
for i = 1:u
    ...
    for j = (pi/8:pi/4/(v-1):3*pi/8)
        ...
        for x = 1:m
            for y = 1:n
                xprime = (x-((m+1)/2))*cos(thetav)+(y-((n+1)/2))*sin(thetav);
                yprime = -(x-((m+1)/2))*sin(thetav)+(y-((n+1)/2))*cos(thetav);
                gFilter(x,y) = (fu^2/(pi*gama*eta))*exp(-((alpha^2)*(xprime^2)+(beta^2)*(yprime^2)))*exp(1i*2*pi*fu*xprime);
            end
        end
        gaborArray{i,acount} = gFilter;
